...
const ASSET_CLASS_COLORS: Record<string, string> = {
  ...
  // The approved shelf groups by product line rather than by asset class, so
  // these three keys reach the same function. They are drawn from the same
  // validated set and take the three most separated slots, because the shelf
  // donut shows exactly these three at once and nothing else. They never
  // co-occur with the asset classes above, which is why slots are reused
  // rather than extending the palette past the hues it was validated for.
  Gold: "var(--cat-1)",
  Commodities: "var(--cat-2)",
  "Mutual Funds": "var(--cat-3)",
};
